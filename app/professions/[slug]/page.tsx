...
function ProficiencyList({ prof }: { prof: ReturnType<typeof getProfession> }) {
  ...
  const hasData =
    p.vitals_skills.length > 0 ||
    p.armaments.length > 0 ||
    p.protection.length > 0 ||
    p.tool_kits.length > 0;
  ...
  if (hasData) {
    const rows = [
      ...
      (r) =>
        r.items.length > 0 && !(r.items.length === 1 && r.items[0] === "-"),
    );
...
function StartingPackList({
  pack,
}: {
  pack: NonNullable<ReturnType<typeof getProfession>>["starting_pack"];
}) {
  ...
  const sections = [
    ...
    (s) =>
      ...
      !(
        s.items.length === 1 &&
        (s.items[0] === "" || s.items[0].toLowerCase() === "none")
      ),
  );
...
export default async function ProfessionDetailPage({ params }: Props) {
  ...
  const baseProfFeats = profFeats.filter(
    (f) => !f.tag || f.tag === f.owner_name,
  );
  const featPathGroups: Record<string, Feat[]> = {};
  profFeats
    .filter((f) => f.tag && f.tag !== f.owner_name)
    .forEach((f) => {
      if (!featPathGroups[f.tag!]) featPathGroups[f.tag!] = [];
      featPathGroups[f.tag!].push(f);
    });

  const hasStartingPack =
    prof.starting_pack.raw || prof.starting_pack.weapons?.items?.length > 0;
  ...
  return (
    <div>
      ...
      >
        <p style={{ color: "var(--text)", lineHeight: 1.65, margin: 0 }}>
          {prof.role}
        </p>
      </div>
      ...
      >
        {prof.starting_vitality && (
          <StatBlock label="Starting Vitality" value={prof.starting_vitality} />
        )}
        {prof.vitality_gained_per_tier && (
          <StatBlock
            label="Vitality per Tier"
        ...
        {prof.body_modifier_bonus && (
          <StatBlock
            label="Body Modifier Bonus"
            value={prof.body_modifier_bonus}
          />
        )}
        {prof.wound_bonus_per_tier && (
          <StatBlock
            label="Wound Bonus per Tier"
            value={String(prof.wound_bonus_per_tier)}
          />
        )}
      </div>
